...
*   **AnythingLLM:** All-in-one AI application (docker/anything-llm/docker-compose.yml)
*   **Nginx Proxy Manager:** For easy reverse proxy and SSL certificate management (docker/proxy/docker-compose.yml)
*   **VS Code Server:** Web-based Visual Studio Code IDE (docker/vscode/docker-compose.yml)
...
*   `/srv/anythingllm/storage`: AnythingLLM storage (documents, vector DBs, etc.).
...
*   `/srv/vscode/config`: User configuration, extensions, and settings for VS Code Server. Note: The actual project files are typically mounted directly from your home directory (e.g., `/home/chrisadmin/workspace`).
